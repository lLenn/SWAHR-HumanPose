...
def do_train(cfg, model, data_loader, loss_factory, optimizer, epoch,
    ...
    for i, (images, heatmaps, masks, joints) in tqdm(enumerate(data_loader)):
        ...
        heatmaps_losses, scale_losses, push_losses, pull_losses = \
        ...
        if i % cfg.PRINT_FREQ == 0 and cfg.RANK == 0:
            ...
            '{heatmaps_loss}{scale_loss}{push_loss}{pull_loss}'.format(
                ...
                scale_loss=_get_loss_info(scale_loss_meter, 'scale'),
            ...
            for idx in range(cfg.LOSS.NUM_STAGES):
                ...
                writer.add_scalar(
                    'train_stage{}_scale_loss'.format(i),
                    heatmaps_loss_meter[idx].val,
                    global_steps
                )
                writer.add_scalar(
